...
**Only edit `*.md` files (English source).** Everything else is generated or derived.

| File | Status | Regenerated by |
|---|---|---|
| `*.md` EN specs | **committed — edit these** | — |
| `images/` | **committed** | — |
| `*.th.md` Thai translations | **committed** (so CI builds without claude) | `mise run translate` after editing EN |
| `*.th.md.hash` | **committed** (idempotency stamps) | `mise run translate` |
| `out/*.pdf` | gitignored — generated | `mise run all` or CI |
| `fonts/` | gitignored — downloaded | `mise run fonts` |
...
4. Uploads 12 PDFs as a workflow artifact (90 day retention)
