...
| Issue | Symptom | Plan |
|---|---|---|
| **CF deploy returning errors today** | `PUT /workers/scripts/quick-compiler -> 502 Bad Gateway` (Ray IDs logged); also seen `fetch failed` on the 24 MB upload. WASM bundle is valid — same artefact also tripped a now-fixed `invalid heap type 'exact'` error before we constrained wasm-opt features. | Retry tomorrow / open CF support ticket with the Ray ID if it persists. Bundle is at the size limit's edge — uploads are bigger than CF normally sees. |
