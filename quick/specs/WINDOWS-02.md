...
## Dimensions, Shopping List + Cost

Measure all openings **brick to brick**. Thai houses use standard openings so these sizes should fit. Frames are ~10-20 mm smaller than the opening (gap filled with foam/silicone).

Auto-generated from `data/windows.json` + `data/scope-picks.json`. Edit either source and run `mise run gen` to regenerate; never edit the included partials by hand. The unit-price columns are *ranges* — custom-order items vary by supplier.

<!-- include: _partials/window-quantity-windows-02.md -->
...
Hand-edited line items — labour:

| Category | Est. Cost (THB) |
|---|---|
| Labour (remove old + install new) | TBC |
| **WINDOWS PHASE 02 TOTAL** | auto-computed supply above + TBC labour |
